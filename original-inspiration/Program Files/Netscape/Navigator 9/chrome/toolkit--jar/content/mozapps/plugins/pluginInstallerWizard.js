...
function nsPluginInstallerWizard(){
  ...
  // arguments[0] is an array that contains three items:
  ...
  if ("arguments" in window) {
    ...
    this.mCallback = window.arguments[0].callback;
  }
...
function wizardFinish(){
  ...
  var success = ((gPluginInstaller.mSuccessfullPluginInstallation > 0) &&
      (gPluginInstaller.mPluginInfoArray.length != 0));
      
  if (gPluginInstaller.mCallback) {
    gPluginInstaller.mCallback(success, gPluginInstaller.mTab);
    
    // clear the tab's plugin list only if we installed at least one plugin
    gPluginInstaller.mTab = null;
    gPluginInstaller.mCallback = null;
  }

...
function onAlertClick(event)
{
  ...
  bag.setProperty("shiftKey", event.shiftKey);
  bag.setProperty("ctrlKey", event.ctrlKey);
  bag.setProperty("metaKey", event.metaKey);
  bag.setProperty("altKey", event.altKey);
  bag.setProperty("button", event.button);
  gAlertListener.observe(bag, "alertclickcallback", gAlertCookie);
}
